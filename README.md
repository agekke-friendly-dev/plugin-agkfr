...
- **Claude Code v2.x 以上**（Plugin システム対応版。新しい方が安定、`npm i -g @anthropic-ai/claude-code` で更新）
...
その後:
- `~/.claude/settings.json` に `agkfr-tools` の記述が残っていれば手動削除
- Claude Code を **`/exit` で再起動**して反映
- （任意）過去ログを残したくない場合: `~/.claude/personal-plugin-data/logs/` を手動削除
- （任意）完全クリーンにしたい場合: `~/.claude/plugins/cache/agkfr-tools/` も削除（残しても害なし、ロールバック用）

> `/plugin uninstall` が `(no content)` で反応しない場合は Claude Code を `/exit` で再起動してから再実行。
